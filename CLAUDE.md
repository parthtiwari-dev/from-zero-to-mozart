...
| `docs/stage-plans/stage-7-research.md` | Stage 7 (Vibrato & expression) — the deep research, **9 fact-passes + a large expression-scoping section** (2026-09-05): the sorting rule (every vibrato claim is about *the waveform* or *the instruction* — never mix them; that's how "vibrato oscillates below the pitch" got into our own docs when the source PDF says "around") · what vibrato is (the pedagogical "roll back from the note" cue vs the acoustic "both above and below, perceived near the mean" — they converge) · rate ~5–7 Hz (Seashore) with the **pulses × BPM ÷ 60 = Hz consistency check** for every drill · width ~0.2–0.35 semitone · Galamian's arm/hand/finger types + the genuine unresolved "which first" teacher disagreement (→ teach arm+wrist, finger REFERENCE) · the readiness gate (hold the violin without the left hand; in-tune first position; has begun shifting — why Stage 7 follows 6) · the prep drills · the "months not weeks" truth · continuous vs terminal vibrato + the fault list · the **ornament→continuous history** (Geminiani 1751 → L. Mozart 1756 → Spohr 1832 → New Grove "ornament until the first quarter of the 20th c." → Ysaÿe/Kreisler → Auer 1921) hedged to "selective and varied, not the modern continuous default" — the K.216 style payoff · the verified Leopold Mozart "Of the Tremolo" passage (→ `LINEAGE-QUOTES.md`). Ends with: the **beat-3 problem** (abcjs renders no vibrato — decide the "hear it" substitute up front), a movement-plan sketch (A–F + P, ~22 lessons), **4 flagged decisions** for the user, and the honest limit (vibrato is the most "a teacher must watch your hand" topic in the course). Status: **research DONE 2026-09-05 — awaiting user sign-off on the 4 decisions before `stage-7.md`.** |
...
- The three **ladder pages** (`sight-reading-cards`, `the-scale-degree-drills`,
  `the-dictation-drills`) and `the-tunes-to-find-by-ear` are **seed sets** — extend with more
...
1. **Bar sums & label counts.** Every `<NotatedExample>` with an explicit `M:` and `L:` — the
   ...
   `labels.length` must equal the notehead count (a chord `[GBd]` = one; a grace note `{c}`
   and a non-`rhythmOnly` rest = none) — a mismatch means unlabelled notes or labels under the
   wrong notes. **`python tools/abc-barcheck.py` checks both** — run it before every commit.
   (Stage 4 shipped with 8 bad bars; the label check found 5 pre-existing mismatches.)
