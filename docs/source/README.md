...
| `Minuet in G (Easy Version in D Major).pdf` | Petzold BWV Anh. 114, first strain only, easy first-position arrangement moved to D major, with string + finger labels ("The Ultimate Songbook for Beginner Violinists" p.41). Raster image inside the PDF — read visually. | **Stage 3** tune `tune-minuet-in-g` (J4) — transcribed 2026-09-03, endings are a reconstruction, verify |
| `Amazing Grace - Intermediate - Violin Solo.pdf` | Violinspiration intermediate arrangement — G major 3/4 first verse (b.1–21), then **modulates to A major with position work + 16ths** (b.22+). Raster. | **Stage 3** tune `tune-amazing-grace` (J4) — we teach the *plain* standard verse (the melody this arrangement decorates); this PDF confirmed the key / tempo / dynamic scheme |
| `gossec-gavotte-score.pdf` | Gossec *Gavotte* (from *Rosine*), full violin+piano score — D major, cut time, *Allegretto molto grazioso*, dense staccato 16ths, grace notes, D.C. al Fine. Raster. | **Stage 3** tune `tune-gossec-gavotte` (J5) — pending; needs MusicXML or a simpler first-position source |
